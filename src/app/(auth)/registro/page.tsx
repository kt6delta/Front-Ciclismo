...
class CiclistaStrategy implements IFormStrategy {
	...
	async handleSubmit(formData: any): Promise<string> {
		...
		const response = await axios.post(
			`${process.env.NEXT_PUBLIC_URL_BACKEND}/registrarUsuarioCiclista`,
			{
				idUsuario: cedula,
				nombre,
				email,
...
export default function Registro() {
	...
	const [formStrategy, setFormStrategy] = useState<IFormStrategy | null>(
		null
	);
	...
	return (
		<>
			<form
				className="w-full p-10 grid gap-4 my-auto"
