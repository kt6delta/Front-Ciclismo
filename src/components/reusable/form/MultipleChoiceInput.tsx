...
export function MultipleChoice({
    ...
    const handleClick = (option: string) => {
        ...
        if (isMultipleAnswer) {
            if (valueAsArray.includes(option)) {
                const newValue = valueAsArray.filter((item) => item !== option)
                stateUpdater!(newValue, index!)
            } else {
                const newValue = [...valueAsArray, option]
                stateUpdater!(newValue, index!)
            }
        } else {
            if (valueAsArray.includes(option)) {
                        ...
                        >
                            <label
                                ...
                                className={`flex items-center justify-center text-lg text-secondary py-3 px-4 border-2 border-secondary-300 rounded-default cursor-pointer w-full text-center 
                                    ...
                                    ${valueAsArray?.includes(option) ? 'bg-primary' : ''} 
                                    outline-secondary`}
                            ...
                            {valueAsArray.includes(option) && optionsToExtraAnswer?.includes(option) && ( // The clicked option require extra answer
                                <div className="mt-4">
